...
namespace LinqExercises.Controllers
{
    public class OrdersController : ApiController
    {
        ...
        {
            // select* from Orders where RequiredDate between '1997-01-01' AND '1997-12-31' AND Freight < 100
            var frightenUnderHundred = from order in _db.Orders
                                       where (order.RequiredDate >= startDate && order.RequiredDate <= endDate && order.Freight<100)
                                       select order;


            return Ok(frightenUnderHundred);
        ...
        public IHttpActionResult PurchaseReport()
        {
            ...
            var productLambda =  _db.Order_Details
                             //    .Where(p=> p.ProductID && p.Quantity)
                                
                                 .Select(p =>  new { product= p.ProductID, QuantityNo= p.Quantity
                                    })
                                     .GroupBy(p => p.product);

            return Ok(productLambda);
